...
serve(async (req) => {
  ...
  try {
    ...
    console.log('📬 Send Client Portal Welcome request:', { projectId, projectName, clientPhone: clientPhone?.slice(0, 5) + '***' })
    ...
    if (!clientPhone || !isValidPhone(clientPhone)) {
      console.error('❌ Invalid or missing phone number:', clientPhone)
      return new Response(
    ...
    if (accessCheckError) {
      console.error('❌ Error checking existing portal access:', accessCheckError)
    ...
    if (existingAccess) {
      console.log('📋 Using existing portal access record:', existingAccess.url_slug)
      urlSlug = existingAccess.url_slug
    } else {
      // Step 2: Create new client_portal_access record (url_slug is auto-generated by trigger)
      console.log('🆕 Creating new client_portal_access record for project:', projectId)
      ...
      console.log('✅ Created portal access with slug:', urlSlug)
    }
    ...
    const smsMessage = `🏠 Welcome to Your Project Portal!
...
- ${companyName}`
        ...
        try {
          ...
          const emailResponse = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              ...
              to: clientEmail,
              subject: `🏠 Welcome to Your Project Portal - ${projectName}`,
              html: `
    ...
    console.log('🎉 Client portal welcome notification complete!')
    ...
    console.error('❌ Unexpected error in send-client-portal-welcome:', error)
    return new Response(
