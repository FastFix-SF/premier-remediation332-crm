 ...
 export const HeroVideo: React.FC<HeroVideoProps> = ({
   ...
   const hero = useHero();
   const configProjects = useProjectShowcase();
   const { data: dbProjects } = usePublicProjects(4);

   const [videoEnded, setVideoEnded] = useState(false);
   const videoRef = useRef<HTMLVideoElement>(null);

   // Use DB projects if available, otherwise fall back to config-based projects
   const { projects, projectImages } = useMemo(() => {
     if (dbProjects && dbProjects.length > 0) {
       const mapped = dbProjects.slice(0, 4).map((p, index) => ({
         id: p.id,
         title: p.name,
         location: p.address || '',
         year: new Date(p.created_at).getFullYear().toString(),
         label: p.project_category || p.project_type || 'Project',
         specs: p.project_type || '',
       }));
       const images = dbProjects.slice(0, 4).map((p, index) =>
         p.best_photo_url || p.ai_image_url || fallbackImages[index % fallbackImages.length]
       );
       return { projects: mapped, projectImages: images };
     }
     return { projects: configProjects, projectImages: fallbackImages };
   }, [dbProjects, configProjects]);
             ...
             >
               {projects.map((project: any, index: number) => (
                 <ProjectCard
                   key={project.id}
                   project={project}
                   image={projectImages[index] || fallbackImages[index % fallbackImages.length]}
                   index={index}
                 />
               ))}
             </div>
